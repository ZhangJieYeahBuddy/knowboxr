...
A general purpose package for Knowbox data analytics. It mainly helps to fetch credentials from online key-value store and establish database connection. You must have `consul` with host, port, and swagger specified in your `R` global environment.

这是作业盒子数据分析团队的 R 功能包，主要用来连接线上的 key-value 存储以及数据库。同时，也支持从 R 环境里启动 `SSH` 反向代理以及下载云端列表。你必须在 R 的全局环境里创建命名 `consul` 的对象，带有连接地址、端口、后缀。
...
For all functions that require to retrieve key-value from Consul, use `get_params()` to obtain information. 
...
Imagine we have to collect data ranged from year 2018 to year 2019. We specify the time-dependent variable and `col_chunks` automatically breaks down the data collection by chunks (default is `weeks`).
...
Currently supports: [Sheetlabs]("https://sheetlabs.com/")
