...
A general purpose package for Knowbox data analytics. It mainly helps to fetch credentials from `Consul` key-value store and establish database connection. You must have `Consul` with host, port, and swagger specified in your `R` global environment.
...
For all functions that require to retrieve key-value from Consul, use `get_params()` to obtain required parameters. 
